...
int execute(int cmdNumber){
    ...
    for(int i = 0; i <= globalCommand.numberOfSimpleCommands; i++){
        ...
        //Setting the output, if it's the last command.
        if (i==globalCommand.numberOfSimpleCommands){
            if(globalCommand.outfile)
                [>
                 * so open() work like this
        ...
        //Creating the child process, the child process will inherit the copies of open file descriptors.
        pid = fork();
        if(pid == 0){
            if(execvp(globalCommand.sCommands[i].arguments[0], globalCommand.sCommands[i].arguments) == -1){
                perror("myShell");
                //exit(EXIT_FAILURE);
            }
        }
